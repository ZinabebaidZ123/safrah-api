...
export const getAllProgram = asyncHandler(async (req, res) => {
  const { limit, pagination, skip } = await setPagination(Program, req);
...
export const deleteSpecificProgram = asyncHandler(async (req, res) => {
  ...
  await Course.deleteMany({ program: program?._id });
  await Student.updateMany(
    { "courses.programID": program?._id },
    { $pull: { courses: { programID: program?._id } } },
    { new: true, runValidators: true }
  )
  await Course.deleteMany({ program: program?._id });
  res.status(StatusCodes.OK)
